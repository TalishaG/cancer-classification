...
```{r}
options(timeout = 600) #Increases the download timeout to 600 seconds in case the GEO dataset download takes longer than the default (important for slow connections)

# Download and prepare data
gse <- getGEO("GSE70947", GSEMatrix = TRUE)[[1]]
```
...
labels <- factor(ifelse(grepl("tumor", pheno$characteristics_ch1.1), 
...
X_all_original <- as.matrix(expr_df[, -ncol(expr_df)])  # DO NOT scale  These are the predictors (all gene expression values)                        
y_all <- expr_df$Class    # response (labels)

cat("Data dims (samples x genes):", dim(X_all_original)[1], "x", dim(X_all_original)[2], "\n")
table(y_all)

X_all <- X_all_original[, colSums(is.na(X_all)) == 0]
...
meta_features_all <- function(X_all,
  ...
  library(factoextra)
   ...
   # Specify filename and dimensions
png(filename = "GSE70947ScreePlot.png", width = 1600, height = 1200, res = 150)
...
dev.off()
  print(fviz_eig(gpca, addlabels = TRUE, ylim = c(0, 95), 
         main = "Scree Plot: Variance Explained by Gene PCs",
         xlab = "Principal Components", ylab = "Percentage of Explained Variance") +
          theme_minimal())
...
# Save the PCA plot as a PNG
ggsave(
  filename = "GSE70947PCA_MetaFeatures.png",  # file name
  plot = pca_plot,                     # the plot object
...
ggsave(
  filename = "cv_metrics_boxplot.png",
